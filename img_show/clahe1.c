...
static void MakeHistogram (kz_pixel_t*, kz_pixel_t*, kz_pixel_t*, unsigned int, unsigned int, unsigned int,
...
const unsigned int uiMAX_RE_X = 16;	  // max. # contextual regions in x-direction
const unsigned int uiMAX_RE_Y = 16;	  // max. # contextual regions in y-direction



/************************** main function CLAHE ******************/
int CLAHE2 (kz_pixel_t* pImage, kz_pixel_t* pImage1, kz_pixel_t* pImage2, unsigned int uiXRes, unsigned int uiYRes,
 ...
 *   uiNrX - Number of contextial regions in the X direction (min 2, max uiMAX_RE_X)
 *   uiNrY - Number of contextial regions in the Y direction (min 2, max uiMAX_RE_Y)
 ...
 */
{
	...
	kz_pixel_t* pImPointer1;
	kz_pixel_t* pImPointer2;
    ...
    if (uiNrX > uiMAX_RE_X) return -1;	   /* # of regions x-direction too large */
    if (uiNrY > uiMAX_RE_Y) return -2;	   /* # of regions y-direction too large */
	...
	for (uiY = 0, pImPointer = pImage, pImPointer1 = pImage1, pImPointer2 = pImage2; uiY < uiNrY; uiY++) 
	{
		for (uiX = 0; uiX < uiNrX; uiX++, pImPointer += uiXSize, pImPointer1 += uiXSize, pImPointer2 += uiXSize) 
		{
            pulHist = &pulMapArray[uiNrBins * (uiY * uiNrX + uiX)];
            MakeHistogram(pImPointer,pImPointer1,pImPointer2,uiXRes,uiXSize,uiYSize,pulHist,uiNrBins,aLUT);
            ClipHistogram(pulHist, uiNrBins, ulClipLimit);
            MapHistogram(pulHist, Min, Max, uiNrBins, ulNrPixels);
        }
        pImPointer += (uiYSize - 1) * uiXRes;		  /* skip lines, set pointer */
		pImPointer1 += (uiYSize - 1) * uiXRes;
		pImPointer2 += (uiYSize - 1) * uiXRes;
    }

    /* Interpolate greylevel mappings to get CLAHE image */
    for (pImPointer = pImage, pImPointer1 = pImage1, pImPointer2 = pImage2, uiY = 0; uiY <= uiNrY; uiY++) 
	{
        if (uiY == 0) 
        ...
        for (uiX = 0; uiX <= uiNrX; uiX++)
		{
			...
			Interpolate(pImPointer1,uiXRes,pulLU,pulRU,pulLB,pulRB,uiSubX,uiSubY,aLUT);
			Interpolate(pImPointer2,uiXRes,pulLU,pulRU,pulLB,pulRB,uiSubX,uiSubY,aLUT);
            pImPointer += uiSubX;			  /* set pointer on next matrix */
			pImPointer1 += uiSubX;
			pImPointer2 += uiSubX;
        }
        pImPointer += (uiSubY - 1) * uiXRes;
		pImPointer1 += (uiSubY - 1) * uiXRes;
		pImPointer2 += (uiSubY - 1) * uiXRes;
    }
...
void MakeHistogram (kz_pixel_t* pImage, kz_pixel_t* pImage1, 
	kz_pixel_t* pImage2, unsigned int uiXRes,
 ...
 */
{
    ...
    for (i = 0; i < uiSizeY; i++) 
	{
        ...
        while (pImage < pImagePointer)
		{
			...
			pulHistogram[pLookupTable[*pImage1++]]++;
			pulHistogram[pLookupTable[*pImage2++]]++;
		}
